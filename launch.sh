...
mapfile -t instanceARR < <(aws ec2 run-instances --image-id $1 --count $2 --instance-type $3 --key-name $6 --security-group-ids $4 --subnet-id $5 --associate-public-ip-address --iam-instance-profile Name=$7 --user-data file://../itmo544-env/install-webserver.sh --output table | grep InstanceId | sed "s/|//g" | tr -d ' ' | sed "s/InstanceId//g")
echo ${instanceARR[@]}
...
#echo $LENGTH	
     ...
     aws rds create-db-instance --db-name itmo544mp1 --db-instance-identifier lsl-db --db-instance-class db.t1.micro --engine MySQL --master-username lisiling --master-user-password ilovebunnies --allocated-storage 5
      fi  
